...
function show(dataghibli) {
  ...
  for (let gh = 0; gh < dataghibli.length; gh++) {
    ...
    sectionMovies.appendChild(info); // Adjuntar la sección de la película al contenedor "gridmovies"
    dataMovies.appendChild(sectionMovies); //appendChild sirve para agregar elementos desde JS a la interfaz sin necesidad de agregarlos en el HTML, lo que resulta es un código simplificado.
  }
}
...
descendingBton.addEventListener("click", () => {
  //Agregar un evento de click.
  const orderReverse = extractTitle(ghibli.films).reverse(); //Extraer los títulos de las películas, ordenar
...
document.querySelector(
  ".modal-paragraph"
).innerHTML = `Did you know that there are six vehicles (whether magical or not) featured in Studio Ghibli films? ${computeVehicle(ghibli.films)} <br><br> Did you know the percentage of female characters (includes humans and other species) featured in Studio Ghibli films is: ${computeFemaleCharacterPercentage(ghibli.films).toFixed(2)} % <br><br> While the percentage of male characters (includes humans and other species) is: ${computeMaleCharacterPercentage(ghibli.films).toFixed(2)} %`; //A continuación la explicación de cada línea:
