...
export function extractTitle(list) {
  //La función se llama "extractTitle" y toma un parámetro llamado "list" (la lista de títulos de las películas)
  return list.sort(function (a, b) {
    //Se usa el método "sort()" en la lista para ordenar sus elementos. Este método toma una función como argumento, que decide cómo se deben ordenar los elementos. La función dentro de "sort()"" compara dos elementos, "a" y "b", basándose en sus títulos.
    if (a.title > b.title) {
      //Si el título de "a" es mayor que el título de "b", devuelve 1, lo que indica que "a" debería ir después de "b" en la ordenación.

      return 1;
    }
    if (a.title < b.title) {
      //Si el título de "a" es menor que el título de "b", devuelve -1, lo que indica que "a" debería ir antes de "b" en la ordenación.
      return -1;
    }
...
export function filterDirector(director) {
  //La Función "filterDirector" toma como parámetro "director". {} es el cuerpo de la función, donde se describe lo que la función hace.
...
export function filterProducer(producer) {
  //Es la misma función que filterDirector
  return ghibli.films.filter((film) => film.producer === producer);
}

export function computeVehicle(list) {
  //La función "computeVehicle" tiene como parámetro "list".
...
export function computeFemaleCharacterPercentage(films) {
  // Inicialización de variables para contar personajes femeninos y totales
  ...
  for (const film of films) {
    // Bucle para recorrer cada película en el arreglo de películas
    for (const character of film.people) {
      // Bucle para recorrer cada personaje en la propiedad 'people' de la película actual
      if (character.gender === "Female") {
        // Verificación de género del personaje: si es 'Female', incrementar contador de personajes femeninos
