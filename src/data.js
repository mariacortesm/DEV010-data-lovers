...
export function computeVehicle(list) {//La funci'on computeVehicle tiene como par'ametro "list".
...
// Para cada elemento en la lista, usamos 'map' para extraer los nombres de los vehículos. La funci'on regresar'a lo siguiente: al objeto "list" le aplicamos el m'etodo "flatMap", el cual recorre cada persona (item) en la lista y hace algo con ellas. Una vez obtenido este nuevo objeto "item", entraremos a su propiedad "vehicles" y aplicaremos el m'etodo "map", el cual revisar'a cada "item" y sacar'a los vehiculos que cada uno de ellos, convirti'endolo en un nuevo objeto llamado "vehicle", del cual obtendremos su propiedad "name". Es decir, todo lo que está dentro de flatMap y map genera una nueva lista que contiene los nombres de todos los nombres de los vehículos de todas las personas y esto es lo que retorna.
...
export function computeMaleCharacterPercentage(films) {//Es lo mismo que female
